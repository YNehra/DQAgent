...
"""
    ...
    try:
        ...
        if "choices" in response_json and response_json["choices"]:
            llm_reply = response_json["choices"][0]["message"]["content"]
            if not llm_reply.strip():  # Check for empty output
                st.warning("⚠️ Cross-table analysis returned an empty output.")
            else:
...
"""
        ...
        try:
            ...
            if "choices" in response_json and response_json["choices"]:
                llm_reply = response_json["choices"][0]["message"]["content"]
                if not llm_reply.strip():  # Check for empty output
                    st.warning(f"⚠️ Analysis for table {table} returned an empty output.")
                else:
...
if mode == "📤 Upload CSV files":
    ...
    if st.button("🔗 Connect & Analyze"):
        ...
        st.success("✅ Databricks analysis complete.")
